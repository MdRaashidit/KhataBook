    ...
    if request.method == "POST":
        ...
        if len(account)>0:
            if ne=='-':
                ...
                transactions_save = Transactions(dbparty=party,dbamount=newAmt,description=adddes,dbdate=adddate,trace=addtrans,credit_debit=addamount)
            ...
            addtrans=request.POST.get('mytrans')
            party = Party.objects.get(id=addparty)
            transactions_save = Transactions(dbparty=party,dbamount=addamount,description=adddes,dbdate=adddate,trace=addtrans,credit_debit=addamount)
            transactions_save.save()
...
def history(request):
    all_transaction=Transactions.objects.all().order_by('-dbdate')
